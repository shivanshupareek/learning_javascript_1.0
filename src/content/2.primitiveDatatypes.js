...
==> When you copy a reference data type, the copy does not have its own data, it only references to the actual data, and
    it is not truly copied and is still connected to the original data.
...
as another extensive example use of the symbol is given below:
...
now as per the example shown above, even though we have two key value pairs with the name of "age", they are both
...
let b = Number.MAX_VALUE //this will show you the largest possible value (1.7976931348623157e+308) regardless of
                               its safety or controllability.


anything we add to the max value will not reflect anything since we are already at the max possible value.

So to control the value which is bigger than the max safe value, we can use BigInt, which simply means that
...
once it is used and done, JavaScript will discard the box object form and return it to a primitive data type.
